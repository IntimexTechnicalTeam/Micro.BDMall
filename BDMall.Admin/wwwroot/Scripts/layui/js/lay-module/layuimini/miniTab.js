 ...
 */
var openTabs = new Array();  // 打开tabs的数据
...
layui.define(["element", "layer", "jquery"], function (exports) {
    ...
    var miniTab = {
         ...
         * @param options.parentId  回调 tab id
        ...
        create: function (options) {
            ...
            // 存储打开tab数据
            if (options.isIframe) openTabs = parent.openTabs;
            openTabs.push({
                id:  options.tabId,
         ...
         * @param tabId
         * @param isParent
         * @param isBatch  是否批量操作tab窗口
         */
        delete: function (tabId, isParent,isBatch) {
            // todo 未知BUG，不知道是不是layui问题，必须先删除元素
            $(".layuimini-tab .layui-tab-title .layui-unselect.layui-tab-bar").remove();

            if (isParent === true) {
                openTabs = parent.openTabs;
                // 处理tab回调
                $.each(openTabs,function(i, item){
                    if(item && item.id === tabId) {
                        if (item.callback && typeof (item.callback) == "function" && !isBatch) {
                            $(".layuimini-tab .layui-tab-title li[lay-id='" + item.parentId + "']", parent.document).click();
                            item.callback();
                        }
                        openTabs.splice(i,1);
                    }
                ...
                $.each(openTabs,function(i, item){
                    if(item && item.id === tabId) {
                        if (item.callback && typeof (item.callback) == "function" && !isBatch) {
                            $(".layuimini-tab .layui-tab-title li[lay-id='" + item.parentId + "']").click();
                            item.callback();
                        }
                        openTabs.splice(i,1);
                    }
        ...
        openNewTabByIframe: function (options) {
            ...
            if (!checkTab) {
                miniTab.create({
                    ...
                    title: options.title,
                    isIframe: true,
                    parentId: $(".layuimini-tab .layui-tab-title li.layui-this", parent.document).attr('lay-id'),
                    callback: options.callback
                });
            }
        ...
        openTabRignMenu: function (tabId, left) {
            ...
            var menuHtml = '<div class="layui-unselect layui-form-select layui-form-selected layuimini-tab-mousedown layui-show" data-tab-id="' + tabId + '" style="left: ' + left + 'px!important">\n' +
                '<dl>\n' +
                '<dd><a href="javascript:;" layuimini-tab-menu-close="current">' + jsMessage.CloseCurrentTab + '</a></dd>\n' +
                '<dd><a href="javascript:;" layuimini-tab-menu-close="other">' + jsMessage.CloseOtherTab + '</a></dd>\n' +
                '<dd><a href="javascript:;" layuimini-tab-menu-close="all">' + jsMessage.CloseAllTabs + '</a></dd>\n' +
                '</dl>\n' +
                '</div>';
            ...
            $('body').on('click', '[layuimini-tab-close]', function () {
                ...
                $(".layuimini-tab .layui-tab-title li").each(function () {
                    ...
                    if (id !== 'layuiminiHomeTabId') {
                        if (closeType === 'all') {
                            miniTab.delete(tabId,false,true);
                        } else {
