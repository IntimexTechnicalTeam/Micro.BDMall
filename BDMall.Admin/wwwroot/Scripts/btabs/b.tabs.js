 ...
 *              修复部分Bug
 * 2017.11.17 - 修正关闭tab时，不应检测title，url
 *              增加关闭方法
 *              增加parentID,关闭tab后，如有parentID,就激活
 */
!function ($) {
    "use strict";
	 ...
	 * @param parentId {string} 从此模块ID打开新的模块
     * @param closeCallback {functino} 關閉tab時，執行的callback操作
    */
    bTabs.prototype.addTab = function (id, title, url, loginCheck, parentId, closeCallback) {
        ...
        openTabs.push(
            {
                id: tabId, parentId: parentId, callback: closeCallback
            });
    ...
    bTabs.prototype.showTab = function (id) {
        console.log("showTab" + id);
        if (!id) console.error('id参数为必须传递参数！');
        var c = constants, $tabs = this.$container, openTabs = this.openTabs, p = this.p;
        id = id.replace(c.prefixKey, "");
        var tabId = c.prefixKey + id;
        if (openTabs && $.isArray(openTabs) && openTabs.length > 0) {
            var exist = false;//是否已存在
            $.each(openTabs, function (i, row) {
    ...
    bTabs.prototype.closeTab = function (id) {
        ...
        //移除内容区
        $(thisTab).remove();
        var a = $('ul.nav-tabs a[href="#' + tabId + '"]', $tabs);
        var li = $(a).closest('li');
        //获得当前tab的前一个tab
        var prevLi = $(li).prev();
        //移除Tab
        ...
        if (tab && tab.parentId) {
            $('ul.nav-tabs a[href="#' + tab.parentId + '"]', $tabs).tab('show');
            //console.log(typeof (tab.callback));
            if (tab.callback && typeof (tab.callback) == "function") {
                tab.callback();
            }
        } else {
	 ...
	 * 新增标签页
	 */
    function bTabsAdd(id, title, url, loginCheck, parentId, callback) {
        return this.each(function () {
            if (!id || !title || !url) return;
            var $this = $(this), data = $this.data('bTabs');
            if (data) data.addTab(id, title, url, loginCheck, parentId, callback);
        });
    }
	/**
	 * 关闭标签页
     ...
     * 显示标签页
     */
    function bTabsShow(id) {
        return this.each(function () {
            if (!id) return;
            var $this = $(this), data = $this.data('bTabs');
            if (data) data.showTab(id);
        });
    }
    var old = $.fn.bTabs;
